...
function App() {
  ...
  const renderAuthLinks = () => {
    ...
    if (user) {
      return (
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <a href={logoutUrl} style={{ color: 'var(--primary-600)', fontWeight: 500 }}>
            Logout
          </a>
        </div>
      );
    ...
    return (
      <a href={loginUrl} style={{ color: 'var(--primary-600)', fontWeight: 500 }}>
        Login with Google
      </a>
    );
  };
  ...
  const renderMainContent = () => {
    ...
    return (
      <h2 style={{ textAlign: 'center', color: 'var(--gray-600)' }}>
        Please log in to build your campaign.
      </h2>
    );
  };
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: 'var(--gray-50)' }}>
      <header style={{ backgroundColor: 'white', borderBottom: '1px solid var(--gray-200)', padding: '0 2rem' }}>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            <Link
              to="/"
              style={{ fontSize: '1.25rem', fontWeight: 'bold', textDecoration: 'none', color: 'var(--gray-900)' }}
            >
              Xeno CRM
            </Link>
            <div style={{ display: 'flex', gap: '0.5rem' }}>
      ...
      <main style={{ maxWidth: '1200px', margin: '2rem auto', padding: '0 2rem' }}>{renderMainContent()}</main>
    </div>
  );
}
